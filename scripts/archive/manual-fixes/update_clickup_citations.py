...
def get_citations_for_audit(audit_id: str) -> List[Dict]:
    """Fetch all citation records linked to the specified audit."""
    url = f'https://api.airtable.com/v0/{BASE_ID}/{CITATIONS_TABLE}'

    # Use filterByFormula to get records linked to this audit
    params = {
...
def main():
    print(f"🔍 Fetching citation records for audit: {AUDIT_ID}\n")

    # Get all citations for this audit
    citations = get_citations_for_audit(AUDIT_ID)
